...
declare var $: any;
@Component({
  selector: 'app-hardcoded-suggestions',
  templateUrl: './hardcoded-suggestions.component.html',
  styleUrls: ['./hardcoded-suggestions.component.scss']
})
export class HardcodedSuggestionsComponent implements OnInit, OnChanges{
    ...
    if (cp && cp.length) {
      this.leadsService.getTopLeadsCp(cp, this.currentReport.vertical).then(data => {
        if (data && data.data) {
          ...
          // this.localStorageService.set(ConstantKeys.CURRENT_LEADS,{leads:this.leadsArray,reportId:this.currentReport._id});
        }
      }).catch(err => {
      });
    }
  ...
  //               .getSearchResult(undefined, 'All Forms', ele.name, undefined, undefined, 1)
  //               .subscribe(data => {
  //                 if (!data.error) {
  ...
  //       });
  //     }
  //     if (this.fillingConnect.length < 5) {
  //       this.secHardcore.forEach(e => {
  //         if (this.fillingConnect.length < 5) {
  //           if (!(_.filter(this.fillingConnect, (filing) => e == filing.name).length)) {
  //             this.secEdgarService
  //               .getSearchResult(undefined, 'All Forms', e, undefined, undefined, 1)
  //               .subscribe(data => {
  //                 if (!data.error) {
  //                   const data1 = JSON.parse(data);
  //                   if (data1.data && data1.data.length) {
  ...
  //       });
  //     }
  //     this.sharedInterconenctService.fillingConnect = this.fillingConnect;
  //   });
  // }
