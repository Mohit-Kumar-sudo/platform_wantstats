...
@Component({
  selector: 'app-competitive-new-development-output',
  templateUrl: './competitive-new-development-output.component.html',
  styleUrls: ['./competitive-new-development-output.component.scss']
})
export class CompetitiveNewDevelopmentOutputComponent implements OnInit {
  ...
  getReportDetailsSuccess(data: any) {
    if (data && data.length) {
      data.forEach(d => {
        if (d.meta_info.section_key === 'newDevelopment') {
          this.secondaryOutputModel = {
            ...
            heading : "New Product/Service Development"
          }
        }
      });
    }
