...
@Component({
  selector: 'app-competitive-market-analysis-output',
  templateUrl: './competitive-market-analysis-output.component.html',
  styleUrls: ['./competitive-market-analysis-output.component.scss']
})
export class CompetitiveMarketAnalysisOutputComponent implements OnInit {
  reportData: ReportDataElement[];
  secondaryOutputModel: any;
  ...
  getReportDetailsSuccess(data: any) {
    if (data && data.length) {
      data.forEach(d => {
        if (d.meta_info.section_key === 'marketAnalysis') {
          this.secondaryOutputModel = {
            ...
            heading : "Market Share/Strategy/Ranking Analysis"
          }
        }
      });
    }
