...
@Component({
  selector: 'app-competitive-joint-ventures-output',
  templateUrl: './competitive-joint-ventures-output.component.html',
  styleUrls: ['./competitive-joint-ventures-output.component.scss']
})
export class CompetitiveJointVenturesOutputComponent implements OnInit {
  ...
  getReportDetailsSuccess(data: any) {
    if (data && data.length) {
      data.forEach(d => {
        if (d.meta_info.section_key === 'jointVenture') {
