...
@Component({
  selector: 'app-competitive-benchmarking-output',
  templateUrl: './competitive-benchmarking-output.component.html',
  styleUrls: ['./competitive-benchmarking-output.component.scss']
})
export class CompetitiveBenchmarkingOutputComponent implements OnInit {
  ...
  getReportDetailsSuccess(data: any) {
    if (data && data.length) {
      data.forEach(d => {
        if (d.meta_info.section_key === 'benchMarking') {
