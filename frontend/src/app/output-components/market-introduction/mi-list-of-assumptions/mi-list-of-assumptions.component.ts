...
@Component({
  selector: 'app-mi-list-of-assumptions',
  templateUrl: './mi-list-of-assumptions.component.html',
  styleUrls: ['./mi-list-of-assumptions.component.scss']
})
export class MiListOfAssumptionsComponent implements OnInit {
  ...
  getReportDetailsSuccess(data: any) {
    if (data && data.length) {
      data.forEach(d => {
        if (d.meta_info.section_key === 'assumptions') {
          this.secondaryOutputModel = {
            ...
            heading : "List of assumptions, Market Introduction"
          }
        }
