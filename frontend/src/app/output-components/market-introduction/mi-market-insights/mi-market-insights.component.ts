...
@Component({
  selector: 'app-mi-market-insights',
  templateUrl: './mi-market-insights.component.html',
  styleUrls: ['./mi-market-insights.component.scss']
})
export class MiMarketInsightsComponent implements OnInit {
  ...
  getReportDetailsSuccess(data: any) {
    if (data && data.length) {
      data.forEach(d => {
        if (d.meta_info.section_key === 'insights') {
          this.secondaryOutputModel = {
            ...
            heading : "Market insights, Market Introduction"
          }
        }
